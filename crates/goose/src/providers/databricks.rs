...
#[async_trait]
impl EmbeddingService for DatabricksProvider {
    fn embedding_capabilities(&self) -> Option<EmbeddingCapabilities> {
        // Databricks supports various embedding models
        Some(EmbeddingCapabilities {
            models: vec![
                EmbeddingModel {
                    name: "databricks-bge-large-en".to_string(),
                    dimensions: 1024,
                    max_input_tokens: 512,
                    cost_per_token: None, // Pricing varies by workspace
                },
            ],
            default_model: "databricks-bge-large-en".to_string(),
            max_batch_size: 100,
    ...
    async fn create_embeddings_with_model(
        ...
        // Use the legacy implementation and wrap in EmbeddingResult
        let embeddings = EmbeddingCapable::create_embeddings(self, texts.clone()).await
        ...
        Ok(EmbeddingResult {
            ...
            model: model_info,
            usage: EmbeddingUsage {
                tokens: None, // Databricks doesn't return token usage for embeddings
